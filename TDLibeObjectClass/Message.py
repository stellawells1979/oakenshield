...
class Message:
    ...
    def __init__(self, data, skip=True):
        '''
        关于重复的消息，你需要将每一个消息ID和对应的聊天ID临时储存起来，skip参数默认为忽略重复的消息，
        如果你有特殊需求，需要重复解析这条消息，请明确给定skip参数为False,为了避免容器过大占用资源，可以按
        实际需要定期清除容器中较早前的历史消息记录
        :param data:
    ...
    def main(self):
        ...
        :return:
        '''
        # 根据self.skip_message参数是否跳过重复的消息ID
...
class MessageLinkInfo(Message):
    ...
    def __init__(self, data):
        super().__init__(data, skip=False)
...
if __name__ == '__main__':
    ...
    from test import message_object

    message = Message(message_object.updateNewMessage_02.get('message'), False)
    message.main()
